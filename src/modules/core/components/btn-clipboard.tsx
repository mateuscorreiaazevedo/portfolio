...
export const ButtonClipboard: React.FC<Props> = props => {
  ...
  const copyToClipboard = () => {
    if (inputRef.current) {
      ...
      navigator.clipboard
        ...
        .then(() => setNotification('Copiado para a área de transferencia', 'success'))
  ...
  return (
    <button className={props.button.className} onClick={copyToClipboard}>
      <input className="bg-transparent w-full cursor-pointer" disabled ref={inputRef} value={props.input.value} />
    </button>
  )
}
